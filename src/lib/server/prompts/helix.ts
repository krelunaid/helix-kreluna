...
export function novaSystemPrompt(language: string, briefLock: string): string {
...
export function atlasSystemPrompt(language: string, briefLock: string): string {
...
${briefLock}
Base the architecture only on the supplied brief and PRD. Do not claim a backend, database, auth provider, or integration is implemented. No markdown.`;
}
...
Return ONLY JSON: {"target":"semantic target","operation":"replace_fragment","before":"one exact unique substring copied from CURRENT HTML","beforeHash":"copy CURRENT_HTML_SHA256 exactly","patch":"replacement substring","validation":["specific check"]}.
