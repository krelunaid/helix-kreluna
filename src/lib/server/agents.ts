    ...
    (input: {
      ...
      projectId:
        typeof input.projectId === "string"
          ? input.projectId.trim().slice(0, 128)
          : "",
      gear: (input.gear === "house" || input.gear === "fast"
        ? input.gear
        : "auto") as Gear,
  ...
  .handler(async ({ data }) => {
    ...
    if (data.sourceJobId) {
      if (
        ...
        const { BuildJobForbiddenError } = await import(
          "@/lib/server/build-job-access"
        );
        throw new BuildJobForbiddenError();
      }
      const { loadBuildJob } = await import("@/lib/server/jobs/queue");
      const { assertGuestBuildAccess, BuildJobForbiddenError } = await import(
        "@/lib/server/build-job-access"
      );
    ...
    const { releaseGuestBudget, reserveGuestAiBudget } = await import(
      "@/lib/server/guest-abuse.server"
    );
    const lease = await reserveGuestAiBudget({ inputBytes });
    let leaseHandled = false;
    try {
      const { createGuestBuildCredential, deriveGuestBuildCredential } = await import(
        "@/lib/server/build-job-access"
      );
      ...
      if (data.sourceJobId) {
        const { createBuildJobDraft } = await import(
          "@/lib/server/jobs/create"
        );
        const { enqueueGuestGateModification } = await import(
          "@/lib/server/review/human-gate"
        );
        const { dispatchBuildJob } = await import(
          "@/lib/server/jobs/dispatch.server"
        );
        ...
        const { enqueueBuild } = await import(
          "@/lib/server/jobs/submit.server"
        );
...
export const getGuestBuildJob = createServerFn({ method: "POST" })
  .validator((input: { jobId: string; guestAccessToken: string }) => ({
    jobId:
      typeof input.jobId === "string" ? input.jobId.trim().slice(0, 128) : "",
    guestAccessToken:
