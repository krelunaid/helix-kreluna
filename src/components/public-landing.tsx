...
/** Public marketing page (hero, house, examples, pricing). Not mounted on `/`. */
export function PublicLanding({ prompt: routePrompt }: { prompt?: string }) {
  const { locale, t } = useI18n();
  const { prompt, setPrompt, busy, build, user } = useHelixCreate(routePrompt);
  const featured = homeFlagshipsFor(locale);
  const showcaseLabels = flagshipShowcaseLabels(locale);

  function onBuild(p: string, gear: "auto" | "house" | "fast", max: boolean, buildLevel: BuildLevel) {
