...
import { GROK_PROVIDERS, authClient, authEnabled, keepSession, signIn } from "@/lib/auth/client";
...
function Login() {
  ...
  const destPath = prompt
    ...
    : next || "/";
  const callbackURL = origin ? `${origin}${destPath.startsWith("/") ? destPath : `/${destPath}`}` : destPath;

  async function onEmail(mode: "in" | "up") {
    setError(null);
    setBusy(mode);
    try {
      if (mode === "up") {
        ...
        if (res.error) throw new Error(res.error.message || t("login.errSignup"));
        ...
        <div className="w-full rounded-xl bg-surface p-8 window-shadow">
          ...
          {authEnabled ? (
            <div className="mt-6 grid gap-2">
              <Button
                ...
                onClick={() => {
                  void signIn("grok-google", { callbackURL, errorCallbackURL: `${origin}/login` }).catch((err) => {
              ...
              {GROK_PROVIDERS.filter((p) => p.providerId !== "grok-google").map((p) => (
                <Button
                  ...
                  onClick={() => {
                    void signIn(p.providerId, { callbackURL, errorCallbackURL: `${origin}/login` }).catch((err) => {
                      setError(err instanceof Error ? err.message : t("login.err"));
                    });
                  }}
                >
          ...
          <form className="grid gap-3" onSubmit={onSubmit}>
            ...
            <Button type="submit" className="w-full" disabled={busy !== null}>
              {busy === "in" ? t("login.wait") : t("login.signin")}
            </Button>
            <Button
              type="button"
              variant="secondary"
            ...
            >
              {busy === "up" ? t("login.wait") : t("login.signup")}
            </Button>
          </form>
...
function GoogleMark() {
  return (
    <svg viewBox="0 0 24 24" className="size-5" aria-hidden>
      ...
      <path fill="#FBBC05" d="M5.84 14.09A6.97 6.97 0 0 1 5.48 12c0-.72.12-1.43.36-2.09V7.07H2.18A10.99 10.99 0 0 0 1 12c0 1.77.42 3.45 1.18 4.93l3.66-2.84z" />
